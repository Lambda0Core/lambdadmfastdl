...
function EFFECT:Init(data)
	...
	if !cvar_muzzle or cvar_muzzle:GetBool() then
		...
		for i = 0, math.random(0,1) do
			self.particle[i] = emitter:Add( "hl1/sprites/muzzleflash1", pos )
			...
			self.particle[i]:SetStartSize( math.Rand(.1, 2) * self.Size )
			self.particle[i]:SetEndSize( math.Rand(8, 14) * self.Size )
