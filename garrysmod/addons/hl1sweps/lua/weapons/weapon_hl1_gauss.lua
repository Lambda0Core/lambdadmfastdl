...
function SWEP:StartFire()
	...
	if self:GetInAttack() != 3 then
		if !fPrimaryFire then
			...
			local fDir = self.forwardDir or Vector()
			local vel = fDir * (flDamage * 5)
			if !self:IsMultiplayerRules() then
...
function SWEP:GaussFire(vecOrigSrc, vecDir, flDamage)
	...
	while flDamage > 10 && nMaxHits > 0 do
		...
		local isValidOwner = IsValid(self.Owner)
		
		if SERVER and isValidOwner then self.Owner:LagCompensation(true) end
		local tr = util.TraceLine({
			start = vecSrc,
			endpos = vecDest,
			mask = traceMask,
		...
		local attacker = isValidOwner and self.Owner or self
		if pEntity:Health() > 0 then
			...
			if isValidOwner and pEntity == self.Owner then
				forceDir = -forceDir -- had to do this to fix owner ragdoll fly forward
			end
