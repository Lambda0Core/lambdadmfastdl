...
MODE.Hooks.PlayerLoadout = function(ply)
    ...
    local tier = math.Clamp(ply:GetScore() + 1, 1, #DMU.Mode.GunTiers)
...
MODE.Hooks.PlayerDeath = function(victim, inflictor, attacker)
    ...
    if victim == attacker and victim:GetScore() > 0 then
        victim:AddScore(-1)
        return
    end
    ...
    if attacker:GetScore() >= #DMU.Mode.GunTiers then
        DMU.EndGame(attacker)
        return
    end

    local tier = math.Clamp(attacker:GetScore() + 1, 1, #DMU.Mode.GunTiers)
